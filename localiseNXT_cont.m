...
% August 2018
% Localisation Function, using 
%
%



function [botEst] = localiseNXT_cont(botSim,map)


debug=0;     %Draws map, particles etc when not equal to one
tic; %start timer

elapsedTime = toc;  %set starting value for elapsedTime
...
%SD = 10;
SD = 100; %Standard dev for weighting calculations
...
moveDistFrac = 0.2;  %fraction of maximum move distance (for bot movement)
randFrac = 0.5;   %fraction denoting likelihood of random turn move (eg 0.9, means random move very unlikely)
topCandidates = round(num/15); %fraction 
...
convergeCheck = 0;   %initialise some variables
iterations=0;
botEst = BotSim(map);
...
OpenUltrasonic(SENSOR_1);   %open ultrasonic sensor port


%% INITIALISE PARTICLES - randomly distrib. particles, with minimum distance to walls
...
for i =1:num
    ...
    particles(i).setScanConfig(particles(i).generateScanConfig(scans))   %set scan configuration to match the number of scans taken by bot.
...
%% 
    ...
    while convergeCheck==0 && elapsedTime<60 %outer loop for iterations - will stop when converged, or timer past 60 sec
        %%
        randVariable=rand();
        %
