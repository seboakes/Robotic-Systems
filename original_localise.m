...
SD = 10;
SD_POS = 3;  % standard deviation position - for convergence check
SD_ANG = 5; 
...
[maxDist, dirIndice] = max(botDist);   %returns maximum distance measured, and an indice to denote the direction.
    ...
    if randVariable<randFrac
        ...
        botSim.turn((dirIndice-1)*(-pi/3));   %turn towards longest distance
        BOT_TURN = (dirIndice-1)*(-pi/3);
       
        botSim.move(moveDistFrac*maxDist);   %move bot a fraction of the previously measured max distance
        ...
        botSim.move(0.9*safeMin);
        BOT_MOVE = 0.9*safeMin;
    end
    
    botInsideMap = botSim.insideMap();   %move bot backwards if it leaves map
    if botInsideMap ==1
    
    else   
        botSim.move(-(moveDistFrac*maxDist));
        BOT_MOVE = 0;
